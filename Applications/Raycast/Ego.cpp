...
Ego::Ego(std::shared_ptr<lazyECS::RenderingSystem> render_sys_in, 
    ...
    renderSys(render_sys_in), physicsSys(physics_sys_in), ego_trans_(transform_in)
{
...
void Ego::sensor_step() {
    ...
    for(int j = 0; j < lidar_vert_count; j++) {
        ...
        for(int i = 0; i < lidar_hor_count; i++) {
            ...
            ray_end.x = ego_pos.x + LIDAR_RANGE * sin(hor_ray_angle*rp3d::PI/180.0f + ego_yaw) * cos(vert_ray_angle*rp3d::PI/180.0f);
            ray_end.y = ego_pos.y + LIDAR_RANGE * sin(vert_ray_angle*rp3d::PI/180.0f);
            ray_end.z = ego_pos.z - 0.3 + LIDAR_RANGE * cos(hor_ray_angle*rp3d::PI/180.0f + ego_yaw) * cos(vert_ray_angle*rp3d::PI/180.0f);

            rp3d::Ray ray(rp3d::Vector3(ego_pos.x, ego_pos.y, ego_pos.z - 0.3), ray_end);
